...
#           |     Procesador de     |    extremos del         |    Xiaomi My Band.     |
...
class ProcesadorAcelerometro:
    ...
    def consume(self):
        try:
            ...
            # Publicador
            channel.queue_declare(queue='body_acelerometro', durable=True)
            channel.basic_qos(prefetch_count=1)
            channel.basic_consume(self.callback, queue='body_acelerometro')
    ...
    def callback(self, ch, method, properties, body):
        ...
        if float(json_message['body_x_position']) > 10 or float(json_message['body_x_position']) < 0 or float(json_message['body_y_position']) < -10 or float(json_message['body_y_position']) > 2 or float(json_message['body_z_position']) > 1 or float(json_message['body_z_position']) < -10:
            monitor = Monitor()
            monitor.print_notification('Se cayo',json_message['datetime'], json_message['id'])
        time.sleep(1)
